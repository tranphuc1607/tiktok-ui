...
function SidebarBetweenWhenLogged() {
    ...
    return ( 
        <div className="mt-[8px] pb-[16px] w-[224px] flex flex-col items-start">
            ...
            <div className="w-full h-[18px] px-[8px] text-[14px] mb-[8px] font-semibold text-colorLetterBold1">Following accounts</div>
            {
                newList.map(list => <ListAccountItem  img = {list.img} key= {list.id} useName = {list.useName} name = {list.name}/>)
            }
            ...
            <button onClick={handlOnclick} className="text-[#fe2c55] mt-[8px] h-[18px] w-[78.5px] leading-[18px] font-semibold text-[14px]">
                See more 
            </button>
        </div>
     );
