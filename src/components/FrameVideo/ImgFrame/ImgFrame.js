...
function ImgFrame({ live, img }) {
    ...
    const wrapAnimation = cx(
        'absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] flex items-center justify-center w-wrapImg h-wrapImg rounded-full ',
        {
            wrapImg: live,
        },
    );
    ...
    return (
        <div className={classLiveAnimate}>
            <div className={wrapAnimation}>
                {live && (
                    <div className="absolute bottom-0 w-[33.7px] h-[15px] bg-[#fe2c55] text-[#ffffff] text-[16px] font-bold text-center leading-[15px]">
                        Live
                    </div>
                )}
